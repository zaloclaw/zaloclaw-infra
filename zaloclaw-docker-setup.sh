...
OPENCLAW_IMAGE="ghcr.io/openclaw/openclaw:2026.3.28"
OPENCLAW_HOME_VOLUME="openclaw_home"

OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$HOME/.openclaw_z}"
OPENCLAW_WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-$HOME/.openclaw_z/workspace}"
OPENCLAW_GATEWAY_PORT="${OPENCLAW_GATEWAY_PORT:-18789}"
OPENCLAW_REMOTE_CDP_URL="${OPENCLAW_REMOTE_CDP_URL:-http://192.168.65.254:9222}"

LITELLM_SETUP_SCRIPT="$ROOT_DIR/llm-setup.sh"
LITELLM_CONFIG_FILE="$ROOT_DIR/litellm-config.yaml"
...
seed_openclaw_config() {
	...
	litellm_api_key="$(read_env_value "LITELLM_API_KEY")"
	if [[ -z "$litellm_api_key" ]]; then
		litellm_api_key="$(read_env_value "LITELLM_MASTER_KEY")"
	fi
	...
	if command -v python3 >/dev/null 2>&1; then
		python3 - "$template_path" "$config_path" "$OPENCLAW_REMOTE_CDP_URL" "$OPENCLAW_GATEWAY_PORT" "$OPENCLAW_WORKSPACE_DIR" "$litellm_api_key" <<'PY'
import copy
import json
import sys

template_path, config_path, cdp_url, gw_port, workspace_dir, litellm_api_key = sys.argv[1:7]
...
def update_litellm_api_key(models_obj):
		...
		if not isinstance(litellm, dict):
				return False
		if litellm.get("apiKey") != "__LITELLM_API_KEY__":
				return False
...
if "skills" not in cfg:
		...
		if isinstance(extra_dirs, list):
				skills.setdefault("load", {})["extraDirs"] = [
						d.replace("__OPENCLAW_WORKSPACE_DIR__", workspace_dir) for d in extra_dirs
				]
		cfg["skills"] = skills
...
PY
	...
	if command -v node >/dev/null 2>&1; then
		node - "$template_path" "$config_path" "$OPENCLAW_REMOTE_CDP_URL" "$OPENCLAW_GATEWAY_PORT" "$OPENCLAW_WORKSPACE_DIR" "$litellm_api_key" <<'NODE'
const fs = require("node:fs");
const [templatePath, configPath, cdpUrl, gwPort, workspaceDir, litellmApiKey] = process.argv.slice(2);
...
const updateLitellmApiKey = (modelsObj) => {
	...
	if (litellm.apiKey !== "__LITELLM_API_KEY__") return false;
	if (!litellmApiKey) return false;
	litellm.apiKey = litellmApiKey;
	return true;
};
...
if (!("skills" in cfg)) {
	...
	if (Array.isArray(extraDirs)) {
		skills.load.extraDirs = extraDirs.map((dir) =>
			dir.replace(/__OPENCLAW_WORKSPACE_DIR__/g, workspaceDir)
		);
	}
	cfg.skills = skills;
	seeded.push("skills");
...
echo "==> Done. Browser deps and Chromium installed in running gateway container."
